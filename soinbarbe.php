<!DOCTYPE html>
<html lang="fr">

<head>

    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="css/soinbarbe.css" />
    <link rel="stylesheet" href="css/font.css" />

    <title>
        Sylv'Barbe
    </title>

</head>

<body>
    <header>
        <?php include_once ('header.php');?>
    </header>

    <main>

        <section id="presentation">

            <h1>
                Soins de la Barbe
            </h1>

        </section>

        <section id="sous-image">

            <p class="blanc citation">
                Nous vous proposons diférents services dans nos salons répartis à travers l'Europe.
            </p>

        </section>

        <section class="section3">
            <p id="para1"> De la crème, du baume en passant par des blaireaux<br> du shampoings et bien d'autre
                cosmetique & accesoires!</p>
            <img src="img/deco2.svg" alt="decorationn en carré coloré" id="deco2svg">
        </section>
        <br>


        <section class="Baume">

            <div class="conteneur">
                <p id="para2">
                    Baume à barbe
                </p>
                <div class="ligneGauche">
                </div>

                <!--  <button type="button">
                <img src="img/fleche.png" alt="fleche noir" id="fleche1">
            </button>-->

                <div class="carteBaume">
                    <figure>
                        <img src="img/coiffant.jpg" alt="baume coiffant barbe" id="coiffant">
                        <figcaption>
                            Baume "Tech Art" <br> Fix Design L'Oréal
                        </figcaption>
                    </figure>

                    <figure>
                        <img src="img/coiffant1.png" alt="baume coiffant barbe">
                        <figcaption>
                            Baume "GoldenBeards" <br> HEARD BALM
                        </figcaption>
                    </figure>

                    <figure>
                        <img src="img/coiffant2.png" alt="baume coiffant barbe" id="coiffant2">
                        <figcaption>
                            Baume "Poraso" <br> Baume pour la barbe
                        </figcaption>
                    </figure>
                </div>

                <input type="button">
                <img src="img/fleche.png" alt="fleche noir" id="fleche2">
                </input>
            </div>
        </section>
        <br>

        <section class="shampoing">


            <div class="conteneur1">
                <p id="para3">
                    Shampoings
                </p>
                <!-- <button>
            <img src="img/fleche.png" alt="fleche noir" id="fleche3">
        </button> -->

                <div class="carteshamp">

                    <figure>
                        <img src="img/shampoing.jpg" alt="shampoing noir" class="imgshamp" id="shampnoir">
                        <figcaption>
                            Shampoing <br> "The barber Company" 250 ml
                        </figcaption>
                    </figure>

                    <figure>
                        <img src="img/shampoing1.png" alt="shampoing noir" class="imgshamp">
                        <figcaption>
                            Shampoing Cheveux Sec au Miel<br>"Osma Tradition Since 1957" 200 ml
                        </figcaption>
                    </figure>

                    <figure>
                        <img src="img/shampoing2.png" alt="shampoing noir" class="imgshamp" id="shampoing2">
                        <figcaption>
                            Shampoing Grey de la marque espagnole Beardburys. <br> Spécialement conçu pour les cheveux blancs et gris
                        </figcaption>
                    </figure>

                </div>

                <button>
                    <img src="img/fleche.png" alt="fleche noir" id="fleche4">
                </button>

                <div class="ligneDroite">
                </div>

            </div>
        </section>



        <section class="huiles">

            <img src="img/deco2.svg" alt="image de décoration orange" id="imgDeco2SVG">

            <p id="para4">
                Huiles
            </p>

            <div class="conteneur2">
                <div class="cartehuile">

                    <!--    <button>
            <img src="img/fleche.png" alt="fleche noir" id="fleche5">
                  </button> -->

                    <figure>
                        <img src="img/huile.PNG" alt="huiles" class="imghuile">
                        <figcaption>Huile à barbe KEUNE<br> traitement au satin 95ml</figcaption>
                    </figure>

                    <figure>
                        <img src="img/huile2.PNG" alt="huiles" class="imghuile">
                        <figcaption>Huile à barbe AZBANE<br> Beard Oil 30ml</figcaption>
                    </figure>


                    <button>
                        <img src="img/fleche.png" alt="fleche noir" id="fleche6">
                    </button>
                </div>
            </div>
        </section>
        <BR>

        <section class="cireBarbe">
            <img src="/img/bois.jpg" alt="planche en bois" id="imgBois">
            <p id="para5">
                Cire à Barbe
            </p>

            <div class="carteCire">
                <figure>
                    <img src="/img/cire1.PNG" alt="cire 1" class="imgCire">
                    <figcaption>
                        Cire Cheveux/Barbe<br> "Le père Lucien" 15g
                    </figcaption>
                </figure>

                <figure>
                    <img src="/img/cire2.PNG" alt="cire 1" class="imgCire">
                    <figcaption>
                        Moustache WAX <br> "Dr.K's 100% Natural Handmade Stylin" 15g
                    </figcaption>
                </figure>
            </div>
        </section>
    </main>
    <!-- <?php include_once ('footer.php');?> -->
</body>

</html>
    